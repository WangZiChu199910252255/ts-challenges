...
type Join<T extends unknown[], U extends string | number, str extends string = ''> = T extends [infer R,...infer S] ? str extends '' ? Join<S,U,`${R&string}`> : Join<S,U,`${str}${U}${R&string}`> : str
